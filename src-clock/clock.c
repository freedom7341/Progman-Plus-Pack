...
// TCHAR   szAlwaysOnTop[]     = TEXT("AlwaysOnTop");
					  ...
					  int nCmdShow)
{
	...
	GetCurrentDirectory (MAX_PATH, szIniFileDir);
	lstrcat(szIniFileDir, TEXT("\\"));
	lstrcat (szIniFileDir, szIniFile);
	...
	ReleaseDC (NULL, hDC);
	if (GetPrivateProfileInt (szSection, szNoShadow, 0, szIniFileDir))
	...
	GetPrivateProfileString (szSection, szOptions, TEXT(""), szParseStr, MAX_PATH, szIniFileDir);
	...
	// if (ClockDisp.bTopMost)
		...
		// AppendMenu (hMenu, MF_ENABLED | MF_CHECKED | MF_STRING, IDM_TOPMOST,
					// szTopmost);
		// SetWindowPos (hWnd, HWND_TOPMOST /*(HWND)-1*/, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
		// }
	// else
	...
	{
		iMaximized = GetPrivateProfileInt (szSection, szMaximized, 0, szIniFileDir);
		if (iMaximized)
...
BOOL InitApplication (HANDLE hInstance)       /* current instance */
{
   WNDCLASS  ClockClass;
	...
	ClockClass.hIcon         = LoadIcon (hInst, cckk);
				...
				case IDM_SETFONT:
				{
					...
					if (ChooseFont (&cf)) {
						...
						WritePrivateProfileString (szSection, szFontFileKey,
										FontStruct.lfFaceName, szIniFileDir);
						bNewFont = TRUE;
...
void NEAR PASCAL ClockPaint (HWND hWnd, register HDC hDC, int hint)
{
  ...
  TCHAR   szTime[6];
  HDC     hTmpDC = NULL;
  HBITMAP hOldBM = NULL; // TODO: is this right?
  HFONT   h;
...
void NEAR PASCAL DrawFatHand (register HDC hDC, int pos, HPEN hPen, BOOL hHand)
{
  ...
  BOOL     fErase;
	...
	{
	  POINT  rgpt[4];
	  HBRUSH hbrInit = SelectObject(hDC, hbrColorWindow); // TODO: is this correct?
...
void NEAR PASCAL ParseSavedWindow (LPTSTR szBuf, PRECT pRect)
{
  PINT  pint;
  int   count;
 ...
 */
	...
	GetCurrentDirectory(MAX_PATH, szIniFileDir); // TODO: check for correctivity!
	lstrcat(szIniFileDir, L"\\");
	lstrcat(szIniFileDir, szIniFile);

	wsprintf (szInt, TEXT("%i"), i);
	WritePrivateProfileString(szSection, szMaximized, szInt, szIniFileDir);

	/* write current clock options */
	PrepareSavedFlags (szInt, &ClockDisp);
	WritePrivateProfileString (szSection, szOptions, szInt, szIniFileDir);

	/* write window position and size */
	PrepareSavedWindow (szInt, &rCoordRect);
	WritePrivateProfileString (szSection, szPosition, szInt, szIniFileDir);

	/* write clock on-top state */
	// PrepareSavedFlags(szInt, &bTopmost);
	// WritePrivateProfileString (szSection, szAlwaysOnTop, szInt, szIniFileDir);

#ifdef JAPAN
	WritePrivateProfileString(
			szSection, szFontFileKey, FontStruct.lfFaceName, szIniFileDir);

	wsprintf (szInt, TEXT("%i"), (INT)FontStruct.lfCharSet);
	WritePrivateProfileString(szSection, szCharSet, szInt, szIniFileDir);
#endif
}
