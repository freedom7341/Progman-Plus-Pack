    ...
    )
{
    ...
    */
    if ( GetWindowLongPtr(hwnd, GWL_ID) != IDC_LED ) {
        return;
    }
    ...
    )
{
    ...
    */
    ...
    yLine = dyLine;
    dwNextTime = GetTickCount64();  // time to do the next scroll, TODO: does it break?
    lScroll = 0;
    iEncrypt = 0;
    ...
    )
{
    ...
    wcscpy_s( lf.lfFaceName, MAX_PATH, g_szAppFontName ); // TODO: safe string functions
...
{
    SetWindowLongPtr( hwnd, GWLP_USERDATA, (LONG)hfont );
    if ( fRedraw ) {
        InvalidateRect( hwnd, NULL, TRUE );
        UpdateWindow( hwnd );
    }
