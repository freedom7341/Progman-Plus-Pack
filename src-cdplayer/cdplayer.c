    ...
    )
{
    ...
    */
    ...
    wcscpy_s( g_szArtistTxt, STR_MAX_STRING_LEN,  IdStr( STR_HDR_ARTIST ) ); // TODO: safe string functions
    wcscpy_s( g_szTitleTxt, STR_MAX_STRING_LEN, IdStr( STR_HDR_TITLE ) );
    wcscpy_s( g_szUnknownTxt, STR_MAX_STRING_LEN, IdStr( STR_UNKNOWN ) );
    wcscpy_s( g_szTrackTxt, STR_MAX_STRING_LEN, IdStr( STR_HDR_TRACK ) );
    ...
    )
{
    ...
    */
	...
	if (cpds.lpData == NULL) {
        ...
        wcscpy_s((LPTSTR)cpds.lpData, 80, GetCommandLine()); // TODO: safe string functions
	...
	_tcsncpy((LPTSTR)cpds.lpData, GetCommandLine(), cchInsert); // TODO: autorun
	((LPTSTR)cpds.lpData)[cchInsert] = 0;
	wcscat_s((LPTSTR)cpds.lpData, 40, g_szUpdateOption); // TODO: does this break shit?
	cchInsert += cchUpdate;
	((LPTSTR)cpds.lpData)[cchInsert] = 0;
	wcscat_s((LPTSTR)cpds.lpData, 40, lpstr);
    ...
    )
{
    ...
    */
    ...
    wcscpy_s( lpstrText, STR_MAX_STRING_LEN, IdStr(STR_NO_CDROMS) ); // TODO: safe string functions
    wcscpy_s( lpstrTitle, STR_MAX_STRING_LEN, IdStr(STR_CDPLAYER) ); // TODO: safe string functions
...
//fix kksuzuka: #2238
    ...
    wcscpy_s( lf.lfFaceName, STR_MAX_STRING_LEN, g_szAppFontName ); // TODO: safe string functions
    ...
    )
{
	...
	*/
	...
	WinHelp(hwnd, g_HelpFileName, HELP_FINDER, 0);
	break;
...
{
    /*
    ** If we are playing or paused and the brain dead "don't stop playing
    ...
    */
	...
	if ( !g_fSelectedOrder || g_fIntroPlay || g_fContinuous
	  || !g_fSingleDisk || !PlayListMatchesAvailList() ) {
	    ...
	    wcscpy_s( s1, STR_MAX_STRING_LEN, IdStr( STR_EXIT_MESSAGE ) ); // TODO: safe string functions
        wcscpy_s( s2, STR_MAX_STRING_LEN, IdStr( STR_CDPLAYER ) );
    ...
    )
{
    ...
    SetWindowLongPtr(hwnd, DWLP_MSGRESULT, ht); // TODO: working? SetWindowLong(Ptr)
    ...
    )
{
	...
	if (g_fTitlebarShowing) {
	    ...
	    uID = SetWindowLongPtr( hwnd, GWL_ID, 0 );
	    ...
	    dwStyle |= dwTransientStyle;
        SetWindowLongPtr( hwnd, GWL_ID, uID );
	    ypos = rc.top - cyMenuCaption;
	    dy = (rc.bottom - rc.top) + cyMenuCaption;
	}
    ...
    )
{
    ...
    wcscpy_s( lpCmdLine, STR_MAX_STRING_LEN, (LPCTSTR)lpcpds->lpData ); // TODO: safe string functions
    ...
    )
{
...
#ifdef DAYTONA
	    // case SC_TASKLIST:
		// id = STR_SYSMENU_SWITCH; HACK: suck my dick
		// break;
#endif
	    }
	    wcscpy_s( szString, STR_MAX_STRING_LEN, IdStr(id) ); // TODO: safe string functions
	}
    ...
    */
	...
	if (flags & MF_SEPARATOR) {
        ...
        wcscpy_s( szString, STR_MAX_STRING_LEN, IdStr(item + MENU_STRING_BASE) ); // TODO: safe string functions
    ...
    )
{
    ...
    */
    wcscpy_s( lpCommandLine, STR_MAX_STRING_LEN, g_szCdplayer ); // TODO: safe string functions
    if ( g_State & (CD_PLAYING | CD_PAUSED) ) {

	CdPlayerStopCmd();
	fWasPlaying = TRUE;

    wcscat_s( lpCommandLine, 80, g_szPlayOption ); // TODO: safe string functions
    }
    ...
    */
    if ( cGoodFiles > 1) {
	wcscat_s( lpCommandLine, 80, g_szTrackOption );
    }
    ...
    */
	...
	if (IsTrackFileNameValid( szFileName, &unused1,
        ...
        wcscat_s( lpCommandLine, MAX_PATH, TEXT("\'") ); // TODO: safe string functions
        wcscat_s( lpCommandLine, MAX_PATH, szFileName );
        wcscat_s( lpCommandLine, MAX_PATH, TEXT("\'") );
        wcscat_s( lpCommandLine, MAX_PATH, g_szBlank );
	}
    }
    ...
    )
{
    ...
    */
    ...
    wcscpy_s( chBuffer2, STR_MAX_STRING_LEN, IdStr(STR_FATAL_ERROR) ); /*"CD Player: Fatal Error"*/
    ...
    )
{
	...
	*/
	...
	_tcsncpy( s, t->name, i );
        ...
        wcscpy_s( s, 80, g_szBlank ); // TODO: safe string functions
	i = 1;
    ...
    )
{
	...
	*/
	if ( (*lpstr == TEXT('\'')) || (*lpstr == TEXT('\"')) ) {
        ...
        wcscpy_s(chOption, 80, lpstr); // TODO: safe string functions
	}
...
#define RIFF_CDDA 0x41444443
    ...
    //
    wcscpy_s(szFileName, MAX_PATH, lpstFileName); // TODO: safe string functions
    ...
    )
{
    ...
    */
    ...
    wcscpy_s( szUnknown, ARTIST_LENGTH, IdStr(STR_UNKNOWN_ARTIST) ); // TODO: safe string functions
	GetPrivateProfileString( szSection, g_szArtist, szUnknown,
				 szArtistName, ARTIST_LENGTH, g_IniFileName );

    wcscpy_s( szFormat, STR_MAX_STRING_LEN, IdStr(STR_DISK_NOT_THERE_K) );
	wsprintf(szText, szFormat, szDiskTitle, szArtistName);
    }
    else {

	wcscpy_s( szText, STR_MAX_STRING_LEN, IdStr(STR_DISK_NOT_THERE) );
    }
    ...
    wcscpy_s( szMsgBoxTitle, STR_MAX_STRING_LEN, IdStr(STR_CDPLAYER) ); // TODO: safe string functions
...
BOOL CheckMCICDA (TCHAR chDrive)
{
	    ...
	    (0xFFFFFFFF == GetFileAttributes (szPath)))
    {
        ...
        wcscpy_s( szTitle, STR_MAX_STRING_LEN, IdStr( STR_MCICDA_MISSING ) ); // TODO: safe string functions
	    wsprintf (szText, szTitle, szPath);
        wcscpy_s( szTitle, STR_MAX_STRING_LEN, IdStr( STR_CDPLAYER ) );
    ...
    if (! hCD)
    {
        ...
        wcscpy_s( szText, STR_MAX_STRING_LEN, IdStr( STR_MCICDA_NOT_WORKING ) ); // TODO: safe string functions
        wcscpy_s( szTitle, STR_MAX_STRING_LEN, IdStr( STR_CDPLAYER ) );
