    ...
    if (NewCdrom != g_LastCdrom) {
        ...
        if (prompt) {
            ...
            if (g_Devices[g_CurrCdrom]->State & CD_PLAYING) {
                ...
                wcscpy_s( s1, STR_MAX_STRING_LEN, IdStr( STR_CANCEL_PLAY ) ); // TODO: safe string functions
                wcscpy_s( s2, STR_MAX_STRING_LEN, IdStr( STR_CHANGE_CDROM ) );
    ...
    if (g_Devices[cdrom]->State &
        ...
        */
        ...
        if (g_Devices[cdrom]->State & CD_IN_USE) {
            wcscpy_s( (LPTSTR)TITLE(cdrom), STR_MAX_STRING_LEN, IdStr(STR_WAITING) ); // TODO: safe string functions
            wcscpy_s( (LPTSTR)ARTIST(cdrom), STR_MAX_STRING_LEN, IdStr(STR_DISC_INUSE) );
        }
        else if (g_Devices[cdrom]->State & CD_BEING_SCANNED) {
            wcscpy_s( (LPTSTR)TITLE(cdrom), STR_MAX_STRING_LEN, IdStr(STR_WAITING) );
            wcscpy_s( (LPTSTR)ARTIST(cdrom), STR_MAX_STRING_LEN, IdStr(STR_BEING_SCANNED) );
        }
        else {
            wcscpy_s( (LPTSTR)TITLE(cdrom), STR_MAX_STRING_LEN, IdStr(STR_INSERT_DISC) );
            wcscpy_s( (LPTSTR)ARTIST(cdrom), STR_MAX_STRING_LEN, IdStr(STR_DATA_NO_DISC) );
        }
    ...
    */
    ...
    if ((cp.Track > 0)&&(cp.Track < 101)) {
        ...
        if ((ABS(Frames - g_Devices[ cdrom ]->CdInfo.iFrameOffset) > 4) &&
            ...
            wcscpy_s( s1, STR_MAX_STRING_LEN, IdStr(STR_BAD_DISC) ); // TODO: safe string functions
    ...
    )
{
    ...
    */
    ...
    wcscpy_s( lpstrText, STR_MAX_STRING_LEN, IdStr(STR_NOT_IN_PLAYLIST) ); // TODO: safe string functions
    wcscpy_s( lpstrTitle, STR_MAX_STRING_LEN, IdStr(STR_CDPLAYER) );
